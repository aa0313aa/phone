...
function doPost(e) {
  try {
    ...
    var userAgent =
      (e &&
        e.headers &&
        (e.headers['User-Agent'] || e.headers['user-agent'])) ||
      '';
    var ip =
      (e &&
        e.headers &&
        (e.headers['X-Forwarded-For'] || e.headers['x-forwarded-for'])) ||
      '';
    sh.appendRow([
      new Date(),
      p['이름'] || '',
...
function debugSendPlain() {
  ...
  var res = UrlFetchApp.fetch(url, {
    method: 'post',
    contentType: 'application/json',
    payload: JSON.stringify({ content: '디버그: ' + new Date().toISOString() }),
    muteHttpExceptions: true,
  });
  Logger.log(
    'debugSendPlain 응답: ' + res.getResponseCode() + ' ' + res.getContentText()
  );
}
function testDiscordNotification() {
  sendDiscordEmbed({
    이름: '테스트',
    연락처: maskPhone('01012345678'),
    문의유형: '비대면 개통',
    통신사: 'KT',
    최근개통일: '2025-11-09',
    미납연체: '없음',
...
function testWebAppPost() {
  ...
  if (!selfUrl) {
    Logger.log('SELF_WEBAPP_URL 속성 없음');
    return;
  }
